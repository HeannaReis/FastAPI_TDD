...
@pytest.fixture
def postgres_client():
    return client.get_base()

@pytest.fixture(autouse=True)
async def clear_collections(postgres_client):
    yield
    collection_names = await postgres_client.get_database().list_collection_names()
    for collection_name in collection_names:
        if collection_name.startswith("system"):
            continue
        await postgres_client.get_database()[collection_name].delete_many({})
...
async def client() -> AsyncGenerator[AsyncClient, None]:
    try:
        ...
        if not isinstance(app, FastAPI):
            raise ValueError("app is not an instance of FastAPI")
    except ImportError as e:
        raise ImportError(f"Error importing app: {e}")
